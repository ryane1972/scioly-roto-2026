...
__attribute__((noinline)) void pivot(float deg = 180.0, double rotVeloTarg = 90, bool toDefAngle = false) {
  ...
  while (true) {
    ...
    if (slowing) {
      veloSet = accelHandle(decelStartSpeed, 3, decelStartPos, 0, dTheta);
    }
    // compute PID
    veloPID.Compute();
...
__attribute__((noinline)) void seek(double x, double y,
  ...
  while (true) {
    ...
    if (distanceToTarget >= 0.1 && fabs(dTheta) >= 10.0) {
    ...
    // Clamp steering
    rotAdjustment = constrain(rotAdjustment, -turnPow, turnPow);
...
void loop() {
  ...
  //forwardT
  seek(1.00, 0.50, .2, 0.15, false, veloConst);
  //forward
  seek(0.96, 0.60, 0.2, 0.05, false, .2);
  seek(0.96, 0.80, 0.2, 0.05, false, .2);
  seek(1.00, 1.00, 0.2, 0.15, false, .2);
  seek(0.50, 1.00, 0.02, 0.01, false, veloConst, true);
  //forwardEnd
  //Done with move, errorCount: 0 , warningCount: 0
